...
public class ManterUsuario extends ManageBeanGeral<Usuario> {
	...
	try {
		...
		result = dao.getSqlExecutor().executarNamedQuery(
			"Dentista.findUser", orm.formatNotNull().values(),
			fields);
	    }
	    if (result != null) {
	...
	obrigatorios
		.add(new ValidadorPadrao("nome", "formUsuario:entradaNome"));
	obrigatorios
		.add(new ValidadorPadrao("user", "formUsuario:entradaUser"));
	obrigatorios.add(new ValidadorPadrao("senha",
		"formDentista:entradaSenha"));
	return null;
    }
