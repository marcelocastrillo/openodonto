...
public class ManterUsuario  extends ManageBeanGeral<Usuario>{
	...
	public String acaoPesquisar() {
		if(this.getBusca().getParams().get("opcao") == null || this.getBusca().getParams().get("opcao").isEmpty() || this.getBusca().getParams().get("opcao").length() < 3){
			getView().addResourceDynamicMenssage("Selecine um filtro de pesquisa.", "formModalUsuario:buscar");
			return DEFAULT_RULE;
		}
		
		if(this.getBusca().getParams().get("param") == null || this.getBusca().getParams().get("param").isEmpty()){
			getView().addResourceDynamicMenssage("Informe o parametro para pesquisa.", "formModalUsuario:buscar");
			return DEFAULT_RULE;
		}
		
		if(this.getBusca().getParams().get("opcao").equals("nome") && this.getBusca().getParams().get("param").length() < 3){
			getView().addResourceDynamicMenssage("Informe pelo menos 3 caracteres.", "formModalUsuario:buscar");
			getBusca().getParams().put("opcao", null);
			return DEFAULT_RULE;
		}
		
		
		if(this.getBusca().getParams().get("opcao").equals("user") && this.getBusca().getParams().get("param").length() < 4){
			getView().addResourceDynamicMenssage("A especialidade deve ter mais que 4 caracteres.", "formModalUsuario:buscar");
			getBusca().getParams().put("opcao", null);
			return DEFAULT_RULE;
		}
		...
		try{
			...
			if(this.getBusca().getParams().get("opcao").equals("codigo")){
				...
				try{
					...
					getView().addResourceDynamicMenssage("Digite Apenas numeros para esta opcao.", "formModalUsuario:buscar");
				...
				result = dao.getSqlExecutor().executarNamedQuery("Usuario.findByNome", orm.formatNotNull().values(), fields);				
			}else if(this.getBusca().getParams().get("opcao").equals("user")){
				usuario.setUser(this.getBusca().getParams().get("param"));
				result = dao.getSqlExecutor().executarNamedQuery("Dentista.findUser", orm.formatNotNull().values(), fields);
		    }
			if(result != null){
				this.getBusca().acaoLimpar();
				getBusca().getResultados().addAll(result);
				getView().addResourceDynamicMenssage("Foram encontrados "+result.size()+" resultados.", "formModalUsuario:buscar");
			}
			else
				getView().addResourceDynamicMenssage("Nao foi encontrado nenhum resultado." , "formModalUsuario:buscar");
		}catch(Exception ex){
			ex.printStackTrace();
			getView().addResourceMessage("ErroSistema." , "formModalUsuario:buscar");
		}finally{
			dao.closeConnection();
		}
		getBusca().getParams().put("param", null);
