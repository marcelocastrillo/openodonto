...
public class ManterDentista extends ManageBeanGeral<Dentista>{
	...
	public String acaoPesquisar() {
		...
		if(this.getBusca().getParams().get("opcao") == null || this.getBusca().getParams().get("opcao").isEmpty() || this.getBusca().getParams().get("opcao").length() < 3){
			getView().addResourceDynamicMenssage("Selecine um filtro de pesquisa.", "formModalDentista:buscar");
			return DEFAULT_RULE;
		}
		
		if(this.getBusca().getParams().get("param") == null || this.getBusca().getParams().get("param").isEmpty()){
			getView().addResourceDynamicMenssage("Informe o parametro para pesquisa.", "formModalDentista:buscar");
			return DEFAULT_RULE;
		}
		
		if(this.getBusca().getParams().get("opcao").equals("nome") && this.getBusca().getParams().get("param").length() < 3){
			getView().addResourceDynamicMenssage("Informe pelo menos 3 caracteres.", "formModalDentista:buscar");
			getBusca().getParams().put("opcao", null);
			return DEFAULT_RULE;
		}
		
		
		if(this.getBusca().getParams().get("opcao").equals("especialidade") && this.getBusca().getParams().get("param").length() < 5){
			getView().addResourceDynamicMenssage("A especialidade deve ter mais que 5 caracteres.", "formModalDentista:buscar");
			getBusca().getParams().put("opcao", null);
			return DEFAULT_RULE;
		}
		...
		try{
			...
			if(this.getBusca().getParams().get("opcao").equals("codigo")){
				...
				try{
					...
					getView().addResourceDynamicMenssage("Digite Apenas numeros para esta opcao.", "formModalDentista:buscar");
					getBusca().getParams().put("param", null);
					getBusca().getParams().put("opcao", null);
					return DEFAULT_RULE;
				}
				dentista.setCodigo(cod);
				IQuery query = CrudQuery.getSelectQuery(Dentista.class, orm.formatNotNull(), fields);
				...
				long cro = 0;
				try{
					cro = Long.parseLong(this.getBusca().getParams().get("param"));
				}catch(Exception ex){
					getView().addResourceDynamicMenssage("Digite Apenas numeros para esta opcao.", "formModalDentista:buscar");
					getBusca().getParams().put("param", null);
					getBusca().getParams().put("opcao", null);
				...
				dentista.setCro(cro);
				result = dao.getSqlExecutor().executarNamedQuery("Dentista.findByCro", orm.formatNotNull().values(), fields);
			}else if(this.getBusca().getParams().get("opcao").equals("especialidade")){
				dentista.setEspecialidade(this.getBusca().getParams().get("param"));
				result = dao.getSqlExecutor().executarNamedQuery("Dentista.findEspecialidade", orm.formatNotNull().values(), fields);
		    }
			if(result != null){
				this.getBusca().acaoLimpar();
				getBusca().getResultados().addAll(result);
				getView().addResourceDynamicMenssage("Foram encontrados "+result.size()+" resultados.", "formModalDentista:buscar");
			}
			else
				getView().addResourceDynamicMenssage("Nao foi encontrado nenhum resultado." , "formModalDentista:buscar");
		}catch(Exception ex){
			ex.printStackTrace();
			getView().addResourceMessage("ErroSistema." , "formModalDentista:buscar");
		}finally{
			dao.closeConnection();
		}
		getBusca().getParams().put("param", null);
	...
	protected List<AbstractValidator> getCamposObrigatorios() {
		...
		obrigatorios.add(new ValidadorPadrao("nome", "formDentista:entradaNome"));	
		obrigatorios.add(new ValidadorPadrao("cro", "formDentista:entradaCro"));
		obrigatorios.add(new ValidadorPadrao("especialidade", "formDentista:entradaEspecialidade"));
		return obrigatorios;
	}
