...
public class DaoPaciente extends DaoBase<Paciente> {
	...
	if (o != null && o.getCodigo() != null
		...
		save = getConnection().setSavepoint(
			"Before Update Paciente - Savepoint");
	...
	try {
	    ...
	    getConnection().setAutoCommit(false);
	    save = getConnection().setSavepoint(
		    "Before Insert Paciente - Savepoint");
	    insert(o);
	    updateRelationshipTelefone(o);
	} catch (Exception ex) {
	    ex.printStackTrace();
	...
	try {
	    getConnection().setAutoCommit(false);
	    save = getConnection().setSavepoint(
		    "Before Remove Paciente - Savepoint");
	    OrmFormat orm = new OrmFormat(o);
	    Map<String, Object> params = orm.formatKey();
	    List<String> referencias = referencedConstraint(Pessoa.class,
		    params);
	    if (referencias.contains(CrudQuery.getTableName(Paciente.class))
		    && referencias.contains(CrudQuery
			    .getTableName(Telefone.class))
		    && referencias.size() == 2) {
		EntityManager<Telefone> entityManagerTelefone = DaoFactory
			.getInstance().getDao(Telefone.class);
		for (Telefone telefone : o.getTelefone()) {
