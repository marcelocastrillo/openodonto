...
public abstract class DaoBase<T extends Entity> implements Serializable,EntityManager<T> {
	...
	private List<Object> getGeneratedValues(PreparedStatement preparedStatement)
			throws SQLException {
	...
	private Map<String, Object> getGeneratedResult(List<String> names,
			List<Object> values) {
	...
	private Map<String, Object> getGeneratedAutoIncremetValues(
			PreparedStatement ps, String table) throws SQLException {
		List<String> names;
	...
	private void applyForwardKey(OrmFormat format, Class<?> type,
			Map<Class<?>, Map<String, Object>> object) {
		if (type.isAnnotationPresent(Inheritance.class)) {
			Class<?> inheriter = format.getOrmResolver().getInheritanceMap()
					.get(type);
			ForwardKey[] fks = type.getAnnotation(Inheritance.class)
					.joinFields();
	...
	public void remove(Map<String, Object> whereParams, boolean permissive)
		...
		if (whereParams == null || whereParams.isEmpty()) {
			throw new RuntimeException(
					"Parametro WHERE obrigatorio. Caso contrario causaria remoção de todos os dados da tabela");
		}
		OrmResolver orm = new OrmResolver(classe);
		LinkedList<Class<?>> sortedSet = new LinkedList<Class<?>>(orm
				.getInheritanceMap().keySet());
		Iterator<Class<?>> iterator = sortedSet.iterator();
		while (iterator.hasNext()) {
			Class<?> type = iterator.next();
			Set<String> keysColumns = orm.format(type, false).keySet();
			Map<String, Object> localParams = disjoinAttributes(keysColumns,
					whereParams, type);
			IQuery query = CrudQuery.getRemoveQuery(localParams, CrudQuery
					.getTableName(type));
			try {
				execute(query);
